// Radicale CalDAV adapter
// Radicale uses default XML namespace (no d: prefix) and serves VTODO
// bodies via GET on .ics files (not inline in PROPFIND).
// User path: / or /<username>/
...
export class RadicaleAdapter implements CalDAVAdapter {
  async getCalendars(
    baseUrl: string,
    username: string,
    password: string,
